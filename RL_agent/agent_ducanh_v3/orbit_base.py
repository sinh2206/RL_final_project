...
LATE_RECAPTURE_2P_ENABLED = True
LATE_RECAPTURE_2P_MIN_PROD = 3
LATE_RECAPTURE_2P_WINDOW = 10
LATE_RECAPTURE_2P_OVERSEND = 2
...
COALITION_MIN_PER_CONTRIBUTOR_2P = 5    
...
ACCUMULATOR_MAX_FEEDS_PER_TURN = 8          
...
BRAIN_LEAD_RESERVE_4P_ONLY = False           
...
TOP1_OPENING_ALLIN_2P_ENABLED = True
TOP1_OPENING_ALLIN_2P_TURN = 30
TOP1_OPENING_ALLIN_2P_MIN_PROD = 3
TOP1_OPENING_ALLIN_2P_MAX_TRAVEL = 22

TOP1_FUNNEL_2P_ENABLED = True
TOP1_FUNNEL_2P_TURN_MIN = 32
TOP1_FUNNEL_2P_MIN_AVAIL = 34
TOP1_FUNNEL_2P_MAX_TRAVEL = 18
TOP1_FUNNEL_2P_MAX_PER_TURN = 3
TOP1_FUNNEL_2P_FRONTIER_IMPROVE = 4.0
...
def segment_hits_sun(x1, y1, x2, y2):
    # High-speed AABB pre-filter: Sun's bounding box is [38.5, 61.5]
    if max(x1, x2) < 38.5 or min(x1, x2) > 61.5 or max(y1, y2) < 38.5 or min(y1, y2) > 61.5:
        return False
...
def aim_at_target(src, target, ships, initial_by_id, ang_vel, world=None):
    ...
    sun OR if convergence isn't reached Ã¢â‚¬â€ better to skip a target than fire
    ...
    at where the target WILL be Ã¢â‚¬â€ fleet flies straight, target swings into
...
def fleet_target_planet(fleet, planets, initial_by_id=None, ang_vel=0.0):
    ...
    against the planet's CURRENT position misses orbital targets Ã¢â‚¬â€ the
...
def forward_project(world, our_capture_target=None, our_capture_turn=None,
    ...
    Inputs:
      world Ã¢â‚¬â€ current World snapshot.
      our_capture_target/turn/ships Ã¢â‚¬â€ optional our planned capture (treated
        as a hypothetical friendly fleet arrival).
      horizon Ã¢â‚¬â€ how many turns to project.
      project_opponent_moves Ã¢â‚¬â€ if True, each enemy planet launches a fraction
      ...
      opponent_emit_fraction Ã¢â‚¬â€ fraction of surplus the projected launch sends.
    Returns:
...
def generate_step_actions(world, max_per_source=3):
    """Generate candidate "step actions" Ã¢â‚¬â€ Melis style. Each step action is
...
def melis_evaluate(world, our_step_action=None, horizon=12, future_horizon=8,
    ...
    Inputs:
      world Ã¢â‚¬â€ current World snapshot.
      our_step_action Ã¢â‚¬â€ optional dict {"target_id", "arrival_turn", "ships"}.
        If provided, simulates our planned capture as part of the projection.
      horizon Ã¢â‚¬â€ short-term sim horizon for our action's effect.
      future_horizon Ã¢â‚¬â€ additional "all-attack-future" projection turns where
      ...
      opp_emit Ã¢â‚¬â€ fraction of surplus opponents launch in projection. 0.30
...
def forward_score(state, player, n_seats, world=None):
    ...
    Combines: ship advantage + 5Ãƒâ€”planet-count advantage + 8Ãƒâ€”production advantage.
    Weights chosen so an extra owned planet is worth ~5 ships (a typical garrison)
    and an extra production unit is worth ~8 ships (Ã¢â€°Ë†2 turns of growth)."""
    n_planets = [0] * n_seats
    n_prod = [0] * n_seats
    n_ships = [0] * n_seats
...
def _detect_mode(world):
    ...
    escalation (10 turns Ã¢â€ â€™ OPPORTUNISTIC, 20 turns Ã¢â€ â€™ PRESSURE). This is the
    Bocsimacko "value action over inaction" principle Ã¢â‚¬â€ patient-vs-patient
...
def _fwd_marginal(world, src_id, angle, ships, player, horizon):
    """V12.8ay: ÃŽâ€ score (with-launch Ã¢Ë†â€™ without-launch) at horizon."""
...
def is_targetable(world, target):
    ...
    every targeting code path obeys it Ã¢â‚¬â€ the previous per-call check at
...
def _update_neutral_watchlist(world):
    ...
    turn is considered wounded Ã¢â‚¬â€ someone else attacked it, so it's now
...
def _neutral_blocked_by_cap(world, target):
    ...
    so a 60-ship neutral about to be hit by enemy 8 Ã¢â€ â€™ effective 52 Ã¢â€ â€™ unblocks."""
...
def friendly_already_committed(world, target_id):
    """Patient ethos: ONE main fleet per target Ã¢â‚¬â€ UNLESS the target is enemy
...
def plan_solo_capture(world, src, tgt, max_avail, max_travel):
    ...
    if SP1_SPEED_AWARE_ENABLED:
        raw_dist = dist(src.x, src.y, tgt.x, tgt.y)
        if raw_dist >= 45.0:
            preferred = max(preferred, min(250, max_avail))
        elif raw_dist >= 25.0:
            preferred = max(preferred, min(80, max_avail))
        elif raw_dist >= SP1_LONG_DIST_THRESHOLD:
    ...
    if (TOP1_OPENING_ALLIN_2P_ENABLED
            and world.is_2p
            and world.step < TOP1_OPENING_ALLIN_2P_TURN
            and tgt.owner == -1
            and int(tgt.production) >= TOP1_OPENING_ALLIN_2P_MIN_PROD
            and turns <= TOP1_OPENING_ALLIN_2P_MAX_TRAVEL
            and max_avail > ships):
        allin = int(max_avail)
        aim_all = aim_at_target(src, tgt, allin, world.initial_by_id,
                                world.ang_vel, world=world)
        if aim_all is not None:
            all_angle, all_turns = aim_all
            if all_turns <= max_travel and all_turns <= TOP1_OPENING_ALLIN_2P_MAX_TRAVEL:
                need_all = effective_needed_to_capture(tgt, all_turns, world)
                if allin >= need_all + margin:
                    ships = allin
                    angle, turns = all_angle, all_turns
...
def _try_late_recapture_2p(world, victim, deadline, available, spent,
                           target_locked, moves, mode_log):
    if not LATE_RECAPTURE_2P_ENABLED or not world.is_2p:
        return False
    if deadline is None:
        return False
    if victim.id in target_locked:
        return False
    if int(victim.production) < LATE_RECAPTURE_2P_MIN_PROD:
        return False

    latest = int(deadline) + LATE_RECAPTURE_2P_WINDOW
    best = None
    for src in world.my_planets:
        if src.id == victim.id:
            continue
        ...
        if avail < MIN_DISPATCH_SHIPS:
            continue
        max_send = int(avail)
        for ships in range(MIN_DISPATCH_SHIPS, max_send + 1):
            aim = aim_at_target(src, victim, ships, world.initial_by_id,
                                world.ang_vel, world=world)
            if aim is None:
                continue
            angle, turns = aim
            if turns <= deadline or turns > latest:
                continue
            owner_at_arrival, ships_at_arrival = predict_defender_at_arrival(
                world, victim, turns
            )
            if owner_at_arrival == world.player:
                continue
            required = int(math.ceil(ships_at_arrival)) + LATE_RECAPTURE_2P_OVERSEND
            if ships < required:
                continue
            score = (
                int(victim.production),
                -int(turns),
                -int(ships),
                int(avail),
                -int(src.id),
            )
            cand = (score, src, angle, turns, ships)
    ...
    if best is None:
        return False
    _score, src, angle, turns, ships = best
    _commit_fleet(world, moves, spent, target_locked,
                  src.id, victim.id, angle, turns, int(ships))
    mode_log[src.id] = "late-recapture-2p"
    mode_log[victim.id] = "late-recapture-target"
    return True
...
def handle_defense(world, rescue_needs, available, spent, target_locked,
    ...
    directly. User-observed scenario: 40 garrison, 10+49 incoming Ã¢â€ â€™ solo
    ...
    for victim_id, (deficit, deadline, victim) in rescue_needs.items():
        ...
        if solo:
            ...
            for _t, src_id, src, _angle_est, avail in solo:
                ...
                if aim_final is None or (deadline is not None and aim_final[1] > deadline):
                    faster = None
                    if deadline is not None and send < avail:
                        for trial in range(int(send) + 1, int(avail) + 1):
                            aim_try = aim_at_target(
                                src, victim, trial,
                                world.initial_by_id, world.ang_vel, world=world
                            )
                            if aim_try is not None and aim_try[1] <= deadline:
                                faster = (trial, aim_try)
                                break
                    if faster is None:
                        last_fail = "doomed-aim-blocked" if aim_final is None else "doomed-too-slow"
                        continue
                    send, aim_final = faster
        ...
        if not COALITION_ENABLED:
            ...
            if _try_late_recapture_2p(world, victim, deadline, available, spent,
                                      target_locked, moves, mode_log):
                continue
            if _try_doom_evac(world, victim, available, spent, target_locked, moves, mode_log):
                continue
...
def _try_doom_evac(world, victim, available, spent, target_locked, moves, mode_log):
    ...
    _enemy_recently_launched (they just emptied Ã¢â€ â€™ weakly defended).
    """
...
def handle_comet_evac(world, available, spent, target_locked, moves, mode_log):
    ...
    that exits the system are lost permanently Ã¢â‚¬â€ evacuation preserves them.
    """
...
def handle_cheap_pickup(world, available, spent, target_locked, moves, mode_log):
    ...
    K=1 nearest is a higher-garrison target. 4P-only Ã¢â‚¬â€ see CHEAP_PICKUP_4P_ONLY.
    """
...
def _effective_target_dist(src, tgt, world):
    ...
    rotating away get longer (demote). One-step approximation Ã¢â‚¬â€ cheap;
...
def _counter_snipe_candidates(world, src, max_travel, target_locked):
    ...
    sorted by re-flip cost ascending. 2P-only Ã¢â‚¬â€ see COUNTER_SNIPE_2P_ONLY note.
    """
...
def _try_coalition_expand(world, src, tgt, max_travel, available, spent,
    ...
    pieces). For tiny targets we DON'T split Ã¢â‚¬â€ the patient ethos prefers
...
def _brain_reserve_lead(world, available, spent, mode_log):
    ...
    and before the expand Ã¢â€ â€™ accumulator Ã¢â€ â€™ mega-hammer chain. If defense
    later commits the same planet, defense overwrites mode_log[p.id] = 'defense'
    and the chain naturally skips it Ã¢â‚¬â€ life beats lead."""
...
def handle_accumulator(world, available, spent, target_locked, moves, mode_log):
    """V14.2 (Phase 3.7, Idea 6c): accumulator Ã¢â‚¬â€ feed surplus from safe
    backline planets to the lead stockpile each turn.

    Engine: fleet speed = 1 + 5Ãƒâ€”(log(ships)/log(1000))^1.5. One big fleet
    ...
    for p in world.my_planets:
        ...
        threat = sum(int(ships) for eta, owner, ships
                     ...
                     if owner != world.player and owner != -1)
        avail = available[p.id] - spent[p.id]
        safety_reserve = ACCUMULATOR_FEEDER_KEEP_RESERVE + int(math.ceil(threat * 1.2))
        surplus = avail - safety_reserve
...
def handle_mega_hammer(world, available, spent, target_locked, moves, mode_log):
    ...
    turns. Launch the ENTIRE garrison as a single huge fleet Ã¢â‚¬â€ exploits
...
def handle_hammer(world, available, spent, target_locked, moves, mode_log):
    ...
    beats defender_at_arrival Ãƒâ€” overkill. Launches stagger so all fleets land
...
def _build_hammer_plan(world, available, spent):
    ...
    or promoted-by-idle. Combined arrival fleet must beat defender Ãƒâ€” overkill.
...
def handle_top1_funnel_2p(world, available, spent, target_locked, moves, mode_log):
    if not TOP1_FUNNEL_2P_ENABLED or not world.is_2p:
        return
    if world.step < TOP1_FUNNEL_2P_TURN_MIN:
        return
    if len(world.my_planets) < 2 or not world.enemy_planets:
        return

    def frontier_distance(p):
        return min(dist(p.x, p.y, e.x, e.y) for e in world.enemy_planets)

    frontier_by_id = {p.id: frontier_distance(p) for p in world.my_planets}
    feeders = []
    for src in world.my_planets:
        if src.id in mode_log:
            continue
        avail = available[src.id] - spent[src.id]
        if avail < TOP1_FUNNEL_2P_MIN_AVAIL:
            continue
        threat = sum(
            int(ships) for eta, owner, ships
            in world.arrivals_by_planet.get(src.id, [])
            if owner != world.player and owner != -1
        )
        if threat > 0:
            continue
        src_frontier = frontier_by_id.get(src.id, 10 ** 9)
        best = None
        for ally in world.my_planets:
            if ally.id == src.id:
                continue
            ally_frontier = frontier_by_id.get(ally.id, 10 ** 9)
            if ally_frontier + TOP1_FUNNEL_2P_FRONTIER_IMPROVE >= src_frontier:
                continue
            aim = aim_at_target(src, ally, avail, world.initial_by_id,
                                world.ang_vel, world=world)
            if aim is None:
                continue
            angle, turns = aim
            if turns > TOP1_FUNNEL_2P_MAX_TRAVEL:
                continue
            score = (
                src_frontier - ally_frontier,
                int(ally.ships) + int(ally.production) * 8,
                -turns,
                -dist(src.x, src.y, ally.x, ally.y),
            )
            if best is None or score > best[0]:
                best = (score, ally, angle, turns)
        if best is None:
            continue
        score, ally, angle, turns = best
        feeders.append((turns, -score[0], -avail, src, ally, angle, int(avail)))

    feeders.sort()
    fed = 0
    for turns, _neg_improve, _neg_avail, src, ally, angle, ships in feeders:
        if fed >= TOP1_FUNNEL_2P_MAX_PER_TURN:
            break
        if src.id in mode_log:
            continue
        remaining = available[src.id] - spent[src.id]
        send = min(int(ships), int(remaining))
        if send < TOP1_FUNNEL_2P_MIN_AVAIL:
            continue
        aim = aim_at_target(src, ally, send, world.initial_by_id,
                            world.ang_vel, world=world)
        if aim is None:
            continue
        angle, turns = aim
        if turns > TOP1_FUNNEL_2P_MAX_TRAVEL:
            continue
        _commit_fleet(world, moves, spent, target_locked,
                      src.id, ally.id, angle, turns, send)
        mode_log[src.id] = "top1-funnel-2p"
        fed += 1
...
def plan_moves(world, deadline=None):
    ...
    if not _over_budget():
        handle_top1_funnel_2p(world, available, spent, target_locked, moves, mode_log)
