...
def score_friendly_targets(world, src, friendly_planets):
    if not ATTN_ENABLED or not friendly_planets:
        return None
    scorer, bonus, extra = _selected_scorer(world)
    rows = []
    valid = []
    for idx, target in enumerate(friendly_planets):
        try:
            raw_dist = base.dist(src.x, src.y, target.x, target.y)
            rows.append(_candidate_features(world, src, target, raw_dist))
            valid.append(target)
        except Exception:
            pass
    if not rows:
        return None
    try:
        raw_scores = scorer.score_many(rows)
        scored = list(zip(raw_scores, valid))
        scored.sort(key=lambda x: -x[0])  # Sort by score descending
        return scored[0][1]  # Return target with highest score
    except Exception as e:
        return None

base.score_friendly_targets = score_friendly_targets
