...
def optimize_fleet_size_for_speed(ships, max_avail):
    if ships <= 0:
        return ships
    thresholds = [1, 11, 43, 137, 385, 1000]
    for t in thresholds:
        if t > ships:
            if t <= max_avail and (t - ships) <= 0.20 * ships:
                return t
            break
    return ships


def aim_for_arrival(world, src, tgt, ships, arrival_abs):
    t_rel_arrival = arrival_abs - world.step
    if t_rel_arrival <= 0:
        return None
    if tgt.id in world.comet_ids:
        tgt_pos = predict_comet_position(tgt.id, world.comets, t_rel_arrival)
    else:
        init_tgt = world.initial_by_id.get(tgt.id)
        if init_tgt is None or is_static_planet(tgt):
            tgt_pos = (tgt.x, tgt.y)
        else:
            tgt_pos = predict_planet_position(tgt, world.initial_by_id, world.ang_vel, t_rel_arrival)
    ...
    for t in range(1, 150):
        launch_abs = arrival_abs - t
        if launch_abs < world.step:
            continue
            
        t_rel_launch = launch_abs - world.step
        init_src = world.initial_by_id.get(src.id)
        if init_src is None or is_static_planet(src):
            src_x, src_y = src.x, src.y
        else:
            src_pos = predict_planet_position(src, world.initial_by_id, world.ang_vel, t_rel_launch)
            if src_pos is None:
                continue
            src_x, src_y = src_pos
            
        est = estimate_arrival(src_x, src_y, src.radius, tgt_x, tgt_y, tgt.radius, ships)
        if est is None:
            continue
        angle, needed_t = est
        if needed_t == t:
...
def plan_solo_capture(world, src, tgt, max_avail, max_travel):
    ...
    ships = optimize_fleet_size_for_speed(ships, max_avail)
...
def handle_comet_intercept(world, available, spent, target_locked, moves, mode_log):
    if not world.comet_ids:
        return

    def has_our_fleet_targeting(world, target_id):
        for c in _pending_commitments:
        ...
        for f in world.fleets:
            if f.owner == world.player:
                ...
                if ftarget is not None and ftarget.id == target_id:
                    return True
        return False

    for c_id in world.comet_ids:
        comet_planet = world.planet_by_id.get(c_id)
        if comet_planet is None:
            continue
        if comet_planet.owner == world.player:
            continue
        if comet_planet.id in target_locked:
            continue
        if has_our_fleet_targeting(world, comet_planet.id):
            continue
        ...
        for src in world.my_planets:
            ...
            aim = aim_at_target(src, comet_planet, avail, world.initial_by_id, world.ang_vel, world=world)
            if aim is None:
                continue
            angle, turns = aim
            if turns > 40:
            ...
            rem_turns = world.comet_remaining.get(comet_planet.id)
            if rem_turns is not None and turns >= rem_turns:
                continue

            need = effective_needed_to_capture(comet_planet, turns, world)
            need = optimize_fleet_size_for_speed(need + 1, avail)
        ...
        if best_src is not None:
            ...
            _commit_fleet(world, moves, spent, target_locked,
                          best_src.id, comet_planet.id, angle, turns, int(best_need))
            mode_log[best_src.id] = "comet-intercept"
...
def handle_accumulator(world, available, spent, target_locked, moves, mode_log):
    ...
    for p in world.my_planets:
        ...
        rem_avail = available[p.id] - spent[p.id]
        just_launched = spent[p.id] > 0 and mode_log.get(p.id) in ("mega-hammer-launched", "hammer", "multiprong")
        
        if enemies:
            min_enemy_dist = min(dist(p.x, p.y, e.x, e.y) for e in enemies)
        else:
            min_enemy_dist = 999.0
            
        is_frontline = min_enemy_dist < 45.0
        is_leader = mode_log.get(p.id) in ("brain-reserved-lead", "accumulator-lead")
        
        urgency = 0.0
        if threat > rem_avail:
            urgency += 1000.0 + (threat - rem_avail)
        if just_launched:
            urgency += 500.0 + spent[p.id]
        if is_frontline:
            urgency += 100.0 + (50.0 - min_enemy_dist)
        if is_leader:
            urgency += 50.0
            
        if threat == 0 and not just_launched and not is_frontline and not is_leader:
            continue
            
        receivers.append((urgency, p))
    ...
    for p in world.my_planets:
        ...
        feeders.append((p, surplus))
        
    if not feeders:
        return
        
    fed_count = 0
    for src, surplus in feeders:
        ...
        for urgency, rx in receivers:
            if rx.id == src.id:
                continue
                
            raw_d = dist(src.x, src.y, rx.x, rx.y)
            if raw_d / MAX_SPEED > ACCUMULATOR_FEEDER_MAX_TRAVEL:
                continue
                
            aim = aim_at_target(src, rx, surplus, world.initial_by_id, world.ang_vel, world=world)
            if aim is None:
                continue
            angle, turns = aim
            
            timing_penalty = 0.0
            enemy_arrivals = [eta for eta, owner, ships in world.arrivals_by_planet.get(rx.id, [])
                              if owner != world.player and owner != -1]
            if enemy_arrivals:
                earliest_enemy_eta = min(enemy_arrivals)
                if turns > earliest_enemy_eta:
                    timing_penalty += 500.0 * (turns - earliest_enemy_eta)
                else:
                    margin_diff = earliest_enemy_eta - turns
                    timing_penalty += 5.0 * margin_diff
            else:
                timing_penalty += 2.0 * turns
                
            score = urgency - timing_penalty
            
            if score > best_score:
                best_score = score
                best_receiver = (rx, angle, turns)
                
        if best_receiver is not None:
            rx, angle, turns = best_receiver
            _commit_fleet(world, moves, spent, target_locked,
                          src.id, rx.id, angle, turns, int(surplus))
            mode_log[src.id] = "accumulator-feeder"
            fed_count += 1
            if rx.id not in mode_log:
                mode_log[rx.id] = "accumulator-lead"
...
def handle_hammer(world, available, spent, target_locked, moves, mode_log):
    ...
    for src_id, launch in list(plan["launches"].items()):
        ...
        if launch["fire_turn_abs"] > world.step:
            continue  
        actual_src_id = int(str(src_id).split("_")[0])
        src = world.planet_by_id.get(actual_src_id)
        if src is None or src.owner != world.player:
            completed_launches.append(src_id)
            continue
        ships = launch["ships"]
        is_split = "_" in str(src_id)
        if not is_split and ships < HAMMER_MIN_PER_CONTRIBUTOR:
            completed_launches.append(src_id)
            continue
        avail = available[actual_src_id] - spent[actual_src_id]
        if avail < ships:
            completed_launches.append(src_id)
            continue
        target = world.planet_by_id[plan["target_id"]]
        target_arrival_abs = launch.get("target_arrival_abs")
        aim = None
        if target_arrival_abs is not None:
            aim = aim_for_arrival(world, src, target, ships, target_arrival_abs)
        if aim is None:
        ...
        angle, turns = aim
        _commit_fleet(world, moves, spent, target_locked,
                      actual_src_id, plan["target_id"], angle, turns, int(ships))
        mode_log[actual_src_id] = "hammer"
        launch["fired"] = True
...
def _build_hammer_plan(world, available, spent):
    ...
    for tgt in targets:
        ...
        for turns, src, ships, angle in chosen:
            ships_bait = int(0.15 * ships)
            ships_main = ships - ships_bait
            staggered = False
            
            if ships_bait >= 2 and ships_main >= 5:
                aim_bait = aim_for_arrival(world, src, tgt, ships_bait, world.step + target_arrival)
                aim_main = aim_for_arrival(world, src, tgt, ships_main, world.step + target_arrival + 1)
                if aim_bait is not None and aim_main is not None:
                    angle_bait, turns_bait = aim_bait
                    angle_main, turns_main = aim_main
                    fire_bait_rel = target_arrival - turns_bait
                    fire_main_rel = target_arrival + 1 - turns_main
                    if fire_bait_rel >= 0 and fire_main_rel >= 0:
                        cand["launches"][f"{src.id}_bait"] = {
                            "fire_turn_abs": world.step + fire_bait_rel,
                            "ships": int(ships_bait),
                            "angle": float(angle_bait),
                            "target_arrival_abs": world.step + target_arrival,
                            "fired": False,
                        }
                        cand["launches"][f"{src.id}_main"] = {
                            "fire_turn_abs": world.step + fire_main_rel,
                            "ships": int(ships_main),
                            "angle": float(angle_main),
                            "target_arrival_abs": world.step + target_arrival + 1,
                            "fired": False,
                        }
                        staggered = True
            ...
            if not staggered:
                cand["launches"][src.id] = {
                    "fire_turn_abs": world.step + (target_arrival - turns),
                    "ships": int(ships),
                    "angle": float(angle),
                    "target_arrival_abs": world.step + target_arrival,
                    "fired": False,
...
def plan_moves(world, deadline=None):
    ...
    if not _over_budget():
        handle_comet_intercept(world, available, spent, target_locked, moves, mode_log)
