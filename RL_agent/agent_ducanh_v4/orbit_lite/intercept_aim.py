...
def _point_to_segment_dist_sq(px: Tensor, py: Tensor, ax: Tensor, ay: Tensor, bx: Tensor, by: Tensor) -> Tensor:
    vx = bx - ax
    vy = by - ay
    wx = px - ax
    wy = py - ay
    vv = vx * vx + vy * vy
    safe_vv = torch.where(vv < 1e-12, torch.ones_like(vv), vv)
    t = ((wx * vx + wy * vy) / safe_vv).clamp(0.0, 1.0)
    cx = ax + t * vx
    cy = ay + t * vy
...
def _analytic_first_contact(
    ...
    * **Narrow phase** — the exact swept-pair only on the shortlisted planets,
    ...
    if px.shape[0] > 1:
        is_orbital = (px[1, :] != px[0, :]) | (py[1, :] != py[0, :])
    else:
        is_orbital = torch.zeros(P, dtype=torch.bool, device=dev)
    ...
    for s in range(0, N, chunk):
        ...
        d1_sq = _point_to_segment_dist_sq(
            pxc[:, :-1, :], pyc[:, :-1, :],
            fx0, fy0, fx1, fy1
        )
        d2_sq = _point_to_segment_dist_sq(
            fx1, fy1,
            pxc[:, :-1, :], pyc[:, :-1, :], pxc[:, 1:, :], pyc[:, 1:, :]
        )
        is_orbital_c = is_orbital[sl].unsqueeze(1)  # [n, 1, K]
        radc_sq = (radc.unsqueeze(1)) ** 2
        hit = (d1_sq < radc_sq) | (is_orbital_c & (d2_sq < radc_sq))
        ...
        # env death: OOB at the new position OR the segment grazes the sun (static) with safety margins.
        nfx = fx[:, 1:]; nfy = fy[:, 1:]; ofx = fx[:, :-1]; ofy = fy[:, :-1]   # [n,H]
        oob_margin = 0.1
        oob = (nfx < oob_margin) | (nfx > BOARD_SIZE - oob_margin) | (nfy < oob_margin) | (nfy > BOARD_SIZE - oob_margin)
        ...
        sun_limit = SUN_RADIUS + 0.25
        sun = ((cxp - CENTER) ** 2 + (cyp - CENTER) ** 2) < (sun_limit * sun_limit)
        env = oob | sun                                                       # [n,H]
