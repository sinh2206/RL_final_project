...
def _estimate_new_fleet_arrivals(
    *,
    ...
    # Phase 1: distance from planet start to fleet segment
    d1_sq = _point_to_segment_distance_sq(
        old_px,
        old_py,
        old_x.unsqueeze(2),
        old_y.unsqueeze(2),
    ...
    # Phase 2: distance from fleet end to planet segment (only for orbital planets)
    d2_sq = _point_to_segment_distance_sq(
        new_x.unsqueeze(2),
        new_y.unsqueeze(2),
        old_px,
        old_py,
    ...
    is_orbital = (planet_x[:, 1:, :] != planet_x[:, :-1, :]) | (planet_y[:, 1:, :] != planet_y[:, :-1, :]) # [N, H, P]
    rad_sq = (radii.view(N, 1, P)) ** 2
    swept_collides = ((d1_sq < rad_sq) | (is_orbital & (d2_sq < rad_sq))) & check_collision
