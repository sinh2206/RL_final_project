...
SEARCH_DISABLES_CHEAP_PICKUP = False  
...
TURN_CUTOFF_STOP_EXPAND_TURN = 170

STAGNATION_RECOVERY_ENABLED = True
STAGNATION_RECOVERY_4P_ONLY = True
STAGNATION_RECOVERY_TURN = 70
STAGNATION_RECOVERY_MAX_PLANETS = 2
STAGNATION_RECOVERY_MIN_PROD_SHARE = 0.30
STAGNATION_RECOVERY_EXTRA_K = 2
STAGNATION_RECOVERY_EXTRA_TRAVEL = 8
STAGNATION_RECOVERY_DISABLE_HAMMER = True
...
class World:
    def __init__(self, obs, inferred_step=None):
        ...
        self.stagnation_recovery = (
            STAGNATION_RECOVERY_ENABLED
            and (not STAGNATION_RECOVERY_4P_ONLY or not self.is_2p)
            and self.step >= STAGNATION_RECOVERY_TURN
            and len(self.my_planets) <= STAGNATION_RECOVERY_MAX_PLANETS
            and self.my_prod_share <= STAGNATION_RECOVERY_MIN_PROD_SHARE
            and any(p.owner == -1 for p in self.planets)
        )
        if self.stagnation_recovery:
            self.turn_cutoff_stop_expand = False
            self.prod_lead_stop_expand_4p = False
...
def handle_expand(world, available, spent, target_locked, moves, mode_log):
    ...
    if world.is_opening:
        ...
        max_travel = world.mode_params["expand_max_travel_mid"]
    if getattr(world, "stagnation_recovery", False):
        K = min(8, K + STAGNATION_RECOVERY_EXTRA_K)
        max_travel = min(60, max_travel + STAGNATION_RECOVERY_EXTRA_TRAVEL)
...
def _hammer_should_fire(world):
    ...
    if STAGNATION_RECOVERY_DISABLE_HAMMER and getattr(world, "stagnation_recovery", False):
        return False
    if world.is_late:
        return True
...
def plan_moves(world, deadline=None):
    ...
    if not _over_budget():
        if not (SEARCH_EXPAND_4P_ENABLED and not world.is_2p
                ...
                and not getattr(world, "stagnation_recovery", False)):
...
def _sanitize_output_moves(world, moves):
    """Final guardrail against invalid actions and overspending."""
    if not isinstance(moves, list):
        return []
    own = {int(p.id): int(p.ships) for p in world.my_planets}
    spent = defaultdict(int)
    out = []
    for mv in moves:
        ...
        try:
            ...
            continue
        if src_id not in own:
            continue
        if not math.isfinite(angle) or ships <= 0:
            continue
        left = own[src_id] - spent[src_id] - 1
        if left <= 0:
            continue
        send = min(left, ships)
        if send <= 0:
            continue
        out.append([src_id, angle, int(send)])
...
def agent(obs, config=None):
    ...
    try:
        ...
        try:
            ...
            act_timeout = 1.0
        soft_budget = max(0.08, min(0.92, act_timeout * SOFT_DEADLINE_FRACTION))
        ...
        return _sanitize_output_moves(world, moves)
    except Exception:
        return []
