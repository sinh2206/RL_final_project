...
# Recovery when 4P openings get stuck with too few planets and over-conservative
# stop-expand gates keep the agent in a long passive loop.
...
class World:
    def __init__(self, obs, inferred_step=None):
        ...
        self.turn_cutoff_stop_expand = (
            ...
            and not self.stagnation_recovery
        )
...
def handle_expand(world, available, spent, target_locked, moves, mode_log):
    ...
    if getattr(world, "stagnation_recovery", False):
        K = K + STAGNATION_RECOVERY_EXTRA_K
        max_travel = max_travel + STAGNATION_RECOVERY_EXTRA_TRAVEL
...
def _nearest_targets(src, world, K, max_travel, target_locked):
    ...
    if (FWD_SIM_RANK_BONUS_4P > 0 and not world.is_2p and len(candidates) > 1):
        ...
        candidates.sort(key=lambda kv: kv[1])
    if TIEBREAK_ENABLED and len(candidates) > 1:
...
MAX_VALID_MOVES_PER_TURN = 24


def _sanitize_moves(world, moves):
    """Final safety net before returning actions to env."""
    if not isinstance(moves, list):
        return []
    owned = {int(p.id): int(p.ships) for p in world.my_planets}
    ...
    for mv in moves:
        ...
        if src_id not in owned:
            continue
        left = owned[src_id] - spent[src_id] - 1
        if left <= 0:
            continue
        ships = min(ships, left)
        if ships <= 0:
            continue
        out.append([src_id, float(angle), int(ships)])
        spent[src_id] += int(ships)
        if len(out) >= MAX_VALID_MOVES_PER_TURN:
            break
    return out
...
def plan_moves(world, deadline=None):
    ...
    return _sanitize_moves(world, moves)
