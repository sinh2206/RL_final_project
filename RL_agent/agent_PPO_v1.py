...
def _fallback_heuristic(world, deadline):
    ...
    for src in my_planets:
        ...
        if available <= FALLBACK_MIN_RESERVE + 1:
            continue
        ...
        if opening:
            ...
            for target in neutrals[:4]:
                ...
                if available - 1 < need + FALLBACK_ATTACK_BUFFER:
                    continue
                send = max(need + FALLBACK_ATTACK_BUFFER, int(available * 0.60))
        ...
        for target in ranked:
            ...
            if owner != player and available - 1 < need + FALLBACK_ATTACK_BUFFER:
                # Do not throw tiny fleets into larger neutral/enemy stacks.
                continue

            if owner == player:
                send = max(1, min(int(available * 0.34), available - 1))
            elif owner == -1:
                send = max(need + FALLBACK_ATTACK_BUFFER, int(available * 0.56))
            else:
                send = max(need + FALLBACK_ATTACK_BUFFER, int(available * 0.64))
            send = max(1, min(send, available - 1))
            if send <= 0:
                continue
    ...
    if len(moves) < FALLBACK_MAX_ORDERS and time.perf_counter() < deadline:
        ...
        for target in non_owned[:4]:
            ...
            for src, send_cap, angle in sorted(source_cands, key=lambda x: x[1], reverse=True)[:FALLBACK_COORD_SOURCES]:
                if remain <= 0 or len(moves) >= FALLBACK_MAX_ORDERS:
                    break
                available = int(src["ships"]) - int(reserved.get(src["id"], 0))
...
def _strong_fallback(obs, config, world, deadline):
    ...
    for name, fn in (
        ...
        if fn is None:
            continue
        try:
            _warn_fallback_once(f"[agent_PPO_v1] Using {name} fallback policy.")
            out = fn(obs, config)
            ...
            _warn_fallback_once(f"[agent_PPO_v1] {name} fallback failed: {exc}")
