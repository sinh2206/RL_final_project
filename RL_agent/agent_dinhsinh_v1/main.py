...
ATTN_ENABLED = True
ATTN_EXPAND_K_EXTRA_2P = 6
ATTN_EXPAND_K_EXTRA_4P = 6
ATTN_BONUS_2P = 1.45
ATTN_BONUS_4P = 1.25
ATTN_MIN_STEP = 2
ATTN_4P_TIE_JITTER = 1e-06
ATTN_MAX_CANDIDATES = 12
ATTN_DEADLINE_BUFFER = 0.055

base.SOFT_DEADLINE_FRACTION = min(getattr(base, "SOFT_DEADLINE_FRACTION", 0.82), 0.70)
...
def _load_scorer(is_2p):
    global scorer_2p, scorer_4p
    key = "2p" if is_2p else "4p"
    if key in _SCORER_FAILED:
        return None
    if is_2p:
        if scorer_2p is None:
            try:
                scorer_2p = importlib.import_module("feature46_weights_2p")
            except Exception:
                _SCORER_FAILED.add(key)
                return None
        return scorer_2p
    if scorer_4p is None:
        try:
            scorer_4p = importlib.import_module("feature46_weights_4p")
        except Exception:
            _SCORER_FAILED.add(key)
            return None
    return scorer_4p
...
def _selected_scorer(world):
    if world.is_2p:
        return _load_scorer(True), ATTN_BONUS_2P, ATTN_EXPAND_K_EXTRA_2P
    return _load_scorer(False), ATTN_BONUS_4P, ATTN_EXPAND_K_EXTRA_4P
...
def _attn_nearest_targets(src, world, K, max_travel, target_locked):
    ...
    if _TURN_DEADLINE and time.perf_counter() >= _TURN_DEADLINE - ATTN_DEADLINE_BUFFER:
        return _base_nearest_targets(src, world, K, max_travel, target_locked)
    scorer, bonus, extra = _selected_scorer(world)
    if scorer is None:
        return _base_nearest_targets(src, world, K, max_travel, target_locked)
    expanded_k = max(K, min(K + extra, ATTN_MAX_CANDIDATES))
...
def _sanitize_moves(obs, moves):
    ...
    for mv in moves:
        ...
        try:
            ...
            continue
        if src not in owned or ships <= 0 or not math.isfinite(angle):
            continue
        left = owned[src] - spent[src] - 1
        if left <= 0:
            continue
        ships = min(ships, left)
        if ships <= 0:
            continue
        out.append([src, angle, int(ships)])
        spent[src] += ships
        if len(out) >= 24:
            break
    return out
...
def _fast_fallback(obs):
    ...
    for row in _read(obs, "planets", []) or []:
        try:
            ...
            continue
    my = sorted((p for p in planets if p["owner"] == player), key=lambda p: -p["ships"])
    ...
    for src in my[:2]:
        ...
        best_score = -1.0e18
        for tgt in targets:
            d = base.dist(src["x"], src["y"], tgt["x"], tgt["y"])
            score = 5.0 * tgt["prod"] - 0.08 * tgt["ships"] - 0.08 * d
            if score > best_score:
                best_score = score
                best = tgt
        if best is None:
            continue
        send = min(avail, max(8, int(best["ships"]) + 1))
...
def agent(obs, config=None):
    global _TURN_DEADLINE
    start = time.perf_counter()
    try:
        act_timeout = float(_read(config, "actTimeout", 1.0) if config is not None else 1.0)
    except Exception:
        act_timeout = 1.0
    _TURN_DEADLINE = start + max(0.08, min(0.78, act_timeout * 0.72))
    try:
        if int(_read(obs, "step", 0) or 0) == 0:
            # Load only the relevant weight module; ducanh_v3 loaded both.
            _load_scorer(_infer_is_2p(obs))
        if time.perf_counter() >= _TURN_DEADLINE:
            return _fast_fallback(obs)
        moves = base.agent(obs, config)
        clean = _sanitize_moves(obs, moves)
        if clean or isinstance(moves, list):
            return clean
        return _fast_fallback(obs)
    except Exception:
        return _fast_fallback(obs)
